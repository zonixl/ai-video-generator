...
SYSTEM_REMOTION_DESIGNER = (
    "你是一个 Remotion 视频编排师。你只输出严格 JSON，不输出 Markdown，不解释。\n"
    "你会根据每个分镜的内容特点，选择最合适的模板，并输出完整的编排指令。\n"
    "你不能设计人物镜头，不能输出 React/CSS/自由坐标。"
)

PLAN_REMOTION_SCENE = """## 任务
把下面这个短视频分镜转换成 Remotion 视频 JSON。只做一个 scene。

## 可用模板（必须从中选择一个）
[需配图] 的模板会自动生成 AI 插画作为背景，你需要提供 visual（英文画面描述）。
[纯文字] 的模板不需要图片，用组件和动画表达内容。

- basic_diagram [纯文字]：图示卡片动画——卡片、箭头、指标、图表等组件的入场动画。适合数据/流程/对比/步骤类内容。
- kinetic_text [纯文字]：逐词灵动文字——文字逐词弹出、上浮、翻转消失。适合金句/观点/短文案/情感类内容。
- image_full [需配图]：全屏背景图——AI 图片铺满屏幕，大字标题叠加。适合风景/故事/宏大主题/纪录片类。
- image_elegant [需配图]：雅致插画——暖色背景+渐变光晕，居中圆角 AI 插画。适合人文/哲理/情感/成长类。
- image_card [需配图]：信息卡片——渐变背景+中央卡片内含 AI 插画。适合科普/知识/解读/教育类。
- image_modern [需配图]：现代科技——深色背景+玻璃卡片内含 AI 插画。适合科技/商业/趋势/互联网类。
- image_neon [需配图]：霓虹赛博——暗色底+霓虹光晕+AI 插画。适合潮流/游戏/音乐/年轻化/娱乐类。

## 模板选择规则
1. 如果内容偏数据、流程、对比、逻辑推理 → basic_diagram
2. 如果内容偏金句、观点、短文案、情绪表达 → kinetic_text
3. 如果内容偏故事、画面感强、需要场景氛围 → 选择 image_* 模板
4. image_* 模板之间，根据内容调性选择：
   - 大气/史诗/自然 → image_full
   - 优雅/文艺/哲理 → image_elegant
   - 专业/科普/教育 → image_card
   - 科技/商业/前沿 → image_modern
   - 潮流/年轻/酷炫 → image_neon
5. 同一视频的不同 scene 可以使用不同模板（混合搭配效果更好）
...
4. 组件数量控制在 3-5 个（宁多勿少），复杂图表模板最多 1 个。每个 scene 必须填满视觉空间。
...
16. 选择 image_* 模板时，必须输出 visual 字段（英文，适合文生图模型的画面描述），components 可以为空数组。
17. 选择 basic_diagram / kinetic_text 模板时，visual 可以省略或为空，必须输出 components。

## 输出 JSON 格式
basic_diagram / kinetic_text 模板（纯文字模板）：
{{
  "scene_index": 1,
  "duration": 5.0,
...
{{
  "scene_index": 1,
  "duration": 5.0,
  "template": "image_elegant",
  "theme": "warm_grid",
  "headline": "阅读改变命运",
  "subtitle": "每天读书一小时，一年后你会感谢自己",
  "visual": "A person sitting in a cozy reading nook by a window, soft golden afternoon light, stack of books, warm atmosphere, cinematic composition",
  "tts_emotion": "温柔亲切",
  "components": []
}}
...
# ============================================================
SYSTEM_KINETIC_PLANNER = (
    "你是一个短视频动态文字编排师。你的任务是为每一句口播文字设计逐词出现在屏幕上的动画节奏。\n"
    "你要像一个导演一样思考：哪些词需要强调、哪些词快速带过、行与行之间如何衔接。\n"
    "你必须输出严格 JSON，不要输出 Markdown 代码块，不要解释。"
)

PLAN_KINETIC_TEXT = """## 任务
为以下口播文字设计逐词动画。文字会按你指定的节奏逐词出现在屏幕上。

## 输入参数
- 总时长：{duration} 秒（这是语音的时长，你的动画必须精确填满这段时间）
- 帧率：{fps} fps
- 语气：{emotion}
- 口播文字：{subtitle}
...
- enter.type: 弹出效果类型
  - pop：弹簧弹出，有弹性感，适合关键词
  - scale_float：放大+上浮，柔和优雅，适合描述性文字
  - slide_up：从下方滑入，干脆利落，适合短词
- enter.scale: 1.2-2.0，初始放大倍数（弹簧动画最终恢复到正常大小）
- enter.dy: -20 ~ -60，上浮距离（像素），负值向上
- exit.type: 行退出效果
  - flip：沿 X 轴翻转消失（当下一行出现时触发）
  - fade：淡出
  - none：不退出（最后一行）

## 规则
1. 按语义自然分词，每行 2-6 个词，总共 1-4 行。
2. 每个词的 start + duration 必须在 {duration} 秒内。最后一个词结束时应接近 {duration}。
3. **关键原则：词的节奏要配合语音节奏** — 重要词停留更久（duration 更长），连接词一闪而过。
4. 动词、数字、转折词（但是、关键、必须）→ scale 更大(1.6-2.0)、stagger 更慢。
5. 语气词、连接词（的、了、和）→ scale 小(1.2-1.3)、速度快。
6. font_size 范围 48-96，关键词更大。
7. color 用 hex 值。关键词用亮色：#f59e0b(金)、#ef4444(红)、#06b6d4(青)、#10b981(绿)。普通词用 #ffffff。
8. y_position 从上到下分布(30-75)，行间距至少 12%。
9. 除了最后一行，其他行 exit 都用 "flip"。
10. word_groups 里每个元素代表一个词或一个固定短语。
